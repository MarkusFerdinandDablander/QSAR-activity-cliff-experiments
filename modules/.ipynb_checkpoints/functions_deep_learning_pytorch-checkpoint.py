...
from torch_geometric.nn import GCNConv, GINConv, global_add_pool, global_max_pool, global_mean_pool
...
# my own functions
from .functions_machine_learning import regression_scores, binary_classification_scores
from .functions_general import arch
...
def fit_pytorch_model(model,
                      ...
                      early_stopping_path = 'checkpoint.pt',
    ...
    """
    Canonical training loop for pytorch deep learning model.
    """
    ...
    # initialise early stopping object (if wanted)
    if early_stopping == True:
        
        early_stopping_object = EarlyStopping(patience = early_stopping_patience, 
    ...
    # define number of trained epochs (this variable changes its value if early stopping is triggered)
    ...
    for epoch in range(num_epochs):
        ...
        # record results and model weights to trigger early stopping (if wanted)
        if early_stopping == True:
            
            # choose loss recorded for early stopping (on test set, or training set if test set is not available)
            if dataset_test != None:
                early_stopping_object(test_loss, model)
            else:
                early_stopping_object(training_loss, model)

            # trigger early stopping for when we run out of patience
            if early_stopping_object.early_stop == True:
                trained_epochs = epoch + 1 - early_stopping_patience
                print("Early stopping after ", trained_epochs, " epochs.")
                break

    # load best model according to early stopping
    if early_stopping == True:
...
def train_mlps_via_optuna(dataset,
    ...
    def objective(trial):
        ...
        (trained_model, 
         ...
         loss_curve_test_set) = fit_pytorch_model(model = model,
                                                  ...
                                                  early_stopping = trial.suggest_categorical("early_stopping", train_hyperparameter_grid["early_stopping"]),
                                                  early_stopping_patience = trial.suggest_categorical("early_stopping_patience", train_hyperparameter_grid["early_stopping_patience"]),
                                                  early_stopping_delta = trial.suggest_categorical("early_stopping_delta", train_hyperparameter_grid["early_stopping_delta"]),
                                                  early_stopping_path = trial.suggest_categorical("early_stopping_path", train_hyperparameter_grid["early_stopping_path"]),
    ...
    # train model with best hyperparameters on whole dataset (no early stopping, but approximately the same number of gradient updates as in best model)
    (trained_best_model,
     trained_epochs,
     loss_curve_training_set,
     loss_curve_test_set) = fit_pytorch_model(model = best_model,
                                              ...
                                              early_stopping = best_trial.params["early_stopping"],
                                              early_stopping_patience = best_trial.params["early_stopping_patience"],
                                              early_stopping_delta = best_trial.params["early_stopping_delta"],
                                              early_stopping_path = best_trial.params["early_stopping_path"],
...
class GCN(nn.Module):
    ...
    def __init__(self,
        ...
        for k in range(n_conv_layers):
            ...
            self.layers.append(GCNConv(in_channels = dim, out_channels = hidden_dim))
...
# training loops for GNN + MLP models
